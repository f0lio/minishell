...
int 	is_it_cmd(char *line)
{
	...
	// note: - if the line ends with backslash '\', we should read another line && push it to the old one then execute.
...
t_cmd 		*parse_line(char *line, t_cmd *commands, t_redirections *redirections, t_separators *separators, t_quote *quotes)
{
	while (*line)
	{
		...
		if (_isalpha(*line))
		{
			// Construct the words in the **args && fill the structure
